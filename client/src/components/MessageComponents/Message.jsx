...
export default function Message({ currentUser, receiverUsername }) {
  ...
  const fetchMessages = async (pageNum) => {
    try {
      const res = await fetch(
        `${API_URL}/messages/${currentUser}/${receiverUsername}?page=${pageNum}&size=20`,
        { credentials: "include" }
      );
      if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        const res = await fetch(`${API_URL}/me/token`, { credentials: "include" });
        if (!res.ok) return;
        const data = await res.json();
  ...
  const sendMessage = () => {
    ...
    stompClient.current.publish({
      ...
      body: JSON.stringify({ receiver: receiverUsername, content: input.trim() }),
    });
    setInput("");
  };
        ...
        {messages.map((msg, idx) => {
          ...
          return (
            <div key={msg.id ?? idx} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-xs px-4 py-2 rounded-2xl text-sm shadow-sm font-poppins
                ${isMine
